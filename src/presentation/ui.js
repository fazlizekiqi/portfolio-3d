...
// ── Welcome overlay (shown once the character finishes assembling) ───────────
// Floating text hovering above the character's head — no card/background,
// just glowing type that belongs to the 3D scene rather than a UI panel.
const welcomeOverlay = document.createElement('div');
welcomeOverlay.id = '_welcome-overlay';
welcomeOverlay.innerHTML = `
  <div id="_welcomeInner">
    <div id="_welcomeTitle">WELCOME</div>
    <div id="_welcomeBody"></div>
  </div>`;
document.body.appendChild(welcomeOverlay);

const welcomeBody = welcomeOverlay.querySelector('#_welcomeBody');

const WELCOME_TEXT =
  `Hi, I'm Fazli — a Senior Software Engineer based in Stockholm.\n` +
  `This isn't a résumé, it's a living 3D experience.\n\n` +
  `Choose PRESENT for a guided tour through my skills, projects & story,\n` +
  `or EXPLORE to walk around my world and discover it at your own pace.`;

let _welcomeTimer  = null;
let _welcomeShown  = false;
let _welcomeAutoHide = null;
...
/* ─── welcome overlay — floating above the character's head, no card ─────── */
#_welcome-overlay {
  position: fixed;
  left: 50%;
  top: 16%;
...
#_welcomeTitle {
  ...
  letter-spacing: .40em;
  margin-bottom: 12px;
  text-shadow: 0 0 22px rgba(0,200,255,0.75), 0 0 46px rgba(0,150,255,0.35);
}
#_welcomeBody {
  color: #dff6ff;
  font-size: 13px;
  ...
  text-shadow:
    ...
    0 0 40px rgba(0,120,220,0.30);
}
@media (max-width: 640px) {
  #_welcome-overlay { top: 12%; width: 92%; }
  #_welcomeTitle { font-size: 11px; letter-spacing: .26em; margin-bottom: 8px; }
  #_welcomeBody  { font-size: 10.5px; line-height: 1.75; }
}
...
export function showWelcomeOverlay() {
  ...
  requestAnimationFrame(() => { welcomeOverlay.style.opacity = '1'; });
  _welcomeTimer = _typeWrite(welcomeBody, WELCOME_TEXT, 14);

  // Auto-dismiss if the visitor never touches PRESENT/EXPLORE.
  if (_welcomeAutoHide) clearTimeout(_welcomeAutoHide);
  _welcomeAutoHide = setTimeout(hideWelcomeOverlay, 18000);
}

export function hideWelcomeOverlay() {
  if (_welcomeAutoHide) { clearTimeout(_welcomeAutoHide); _welcomeAutoHide = null; }
  if (!_welcomeShown) return;
  _welcomeShown = false;
